...
class Hook(BaseHook):
    def __init__(self, state, mqtt_client, config):
        ...
        self.button_topic = self.config.get(
            "button_topic", "zigbee2mqtt/home/living_room/switch/lamps/action"
        )
    ...
    async def on_message(self, topic: str, payload: str):
        ...
        if topic == self.button_topic:
            if payload == "single":
                ...
                await self.state.set(StateKey.BTN_LVNGRM_SGLCLK, time.time())

            elif payload == "double":
                logger.info("livingroom button double-pressed")
                await self.state.set(StateKey.BTN_LVNGRM_DBLCLK, time.time())
