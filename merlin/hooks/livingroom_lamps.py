...
class Hook(BaseHook):
    def __init__(self, state, mqtt_client, config):
        ...
        self.lamp1_topic = self.config.get("lamp1_topic", "zigbee2mqtt/home/living_room/plug/lamp_1")
        self.lamp2_topic = self.config.get("lamp2_topic", "zigbee2mqtt/home/living_room/plug/lamp_2")
        self.set_topic = self.config.get("set_topic", "zigbee2mqtt/living_room_lamps/set")
    ...
    async def on_state_change(self, key: str, old_value, new_value):
        if key in (StateKey.LIVINGROOM_BUTTON_SINGLE_TS, StateKey.LIVINGROOM_BUTTON_DOUBLE_TS):
            await self._toggle_lamps()

        elif key == StateKey.USER_AT_HOME and new_value is False:
